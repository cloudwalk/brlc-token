...
describe("Contract 'ERC20Freezable'", async () => {
  ...
  describe("Function 'approveFreezing()'", async () => {
    it("Approves freezing and emits the correct event", async () => {
      ...
      await expect(token.connect(user1).approveFreezing())
        .to.emit(token, EVENT_NAME_FREEZE_APPROVAL).withArgs(user1.address);
  ...
  describe("Function 'freeze()'", async () => {
    it("Freezes tokens and emits the correct events", async () => {
      ...
      await expect(token.connect(blacklister).freeze(user1.address, TOKEN_AMOUNT))
        .to.emit(token, EVENT_NAME_FREEZE).withArgs(user1.address, TOKEN_AMOUNT, 0);
      ...
      await expect(token.connect(blacklister).freeze(user1.address, TOKEN_AMOUNT + 1))
        .to.emit(token, EVENT_NAME_FREEZE).withArgs(user1.address, TOKEN_AMOUNT + 1, TOKEN_AMOUNT);
      ...
      await expect(token.connect(blacklister).freeze(user1.address, TOKEN_AMOUNT - 2))
        .to.emit(token, EVENT_NAME_FREEZE).withArgs(user1.address, TOKEN_AMOUNT - 2, TOKEN_AMOUNT + 1);
  ...
  describe("Function 'transferFrozen()'", async () => {
    it("Transfers frozen tokens and emits correct events", async () => {
      ...
      await expect(token.connect(blacklister).transferFrozen(user1.address, user2.address, TOKEN_AMOUNT))
        .to.emit(token, EVENT_NAME_FREEZE_TRANSFER).withArgs(user1.address, TOKEN_AMOUNT)
        .to.emit(token, EVENT_NAME_FREEZE).withArgs(user1.address, TOKEN_AMOUNT, 0)
    ...
    it("Is reverted if there is a lack of frozen balance", async () => {
      ...
      await proveTx(token.connect(user1).approveFreezing());
      await proveTx(token.connect(blacklister).freeze(user1.address, TOKEN_AMOUNT));
      await expect(token.connect(blacklister).transferFrozen(user1.address, user2.address, TOKEN_AMOUNT + 1))
  ...
  describe("Frozen token scenarios", async () => {
    ...
    it("Tokens below the frozen balance cannot be transferred successfully", async () => {
      ...
      await proveTx(token.connect(user1).approveFreezing());
      await proveTx(token.connect(blacklister).freeze(user1.address, TOKEN_AMOUNT));
      await expect(token.connect(user1).transfer(user2.address, 2))
