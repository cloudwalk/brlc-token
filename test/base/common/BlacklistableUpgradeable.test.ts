...
describe("Contract 'BlacklistableUpgradeable'", async () => {
  ...
  async function deployAndConfigureBlacklistable(): Promise<{ blacklistable: Contract }> {
  ...
  describe("Function 'initialize()'", async () => {
    ...
    it("Is reverted if the implementation contract is called even for the first time", async () => {
      ...
      await expect(
        blacklistableImplementation.initialize()
      ).to.be.revertedWith(REVERT_MESSAGE_INITIALIZABLE_CONTRACT_IS_ALREADY_INITIALIZED);
    });
    ...
    it("Is reverted if the internal initializer is called outside of the init process", async () => {
      const { blacklistable } = await setUpFixture(deployBlacklistable);
      await expect(
        blacklistable.call_parent_initialize()
    ...
    it("Is reverted if the internal unchained initializer is called outside of the init process", async () => {
      const { blacklistable } = await setUpFixture(deployBlacklistable);
      await expect(
        blacklistable.call_parent_initialize_unchained()
      ).to.be.revertedWith(REVERT_MESSAGE_INITIALIZABLE_CONTRACT_IS_NOT_INITIALIZING);
    });
  });
  ...
  describe("Function 'setBlacklister()'", async () => {
    it("Executes as expected and emits the correct event", async () => {
      ...
      await expect(
        blacklistable.connect(deployer).setBlacklister(blacklister.address)
      ).to.emit(
        blacklistable,
      ...
      await expect(
        blacklistable.connect(deployer).setBlacklister(blacklister.address)
      ).not.to.emit(blacklistable, EVENT_NAME_BLACKLISTER_CHANGED);
    });

    it("Is reverted if called not by the owner", async () => {
      const { blacklistable } = await setUpFixture(deployBlacklistable);
      await expect(
        blacklistable.connect(user).setBlacklister(user.address)
  ...
  describe("Function 'blacklist()'", async () => {
    it("Executes as expected and emits the correct event if it is called by the blacklister", async () => {
      ...
      await expect(
        blacklistable.connect(blacklister).blacklist(user.address)
      ).to.emit(
        blacklistable,
      ...
      await expect(
        blacklistable.connect(blacklister).blacklist(user.address)
      ).not.to.emit(blacklistable, EVENT_NAME_BLACKLISTED);
    });

    it("Is reverted if called not by the blacklister", async () => {
      const { blacklistable } = await setUpFixture(deployAndConfigureBlacklistable);
      await expect(
        blacklistable.connect(user).blacklist(user.address)
  ...
  describe("Function 'unBlacklist()'", async () => {
    it("Executes as expected and emits the correct event if it is called by the blacklister", async () => {
      ...
      expect(await blacklistable.isBlacklisted(user.address)).to.equal(true);
      await expect(
        blacklistable.connect(blacklister).unBlacklist(user.address)
      ).to.emit(
        blacklistable,
      ...
      await expect(
        blacklistable.connect(blacklister).unBlacklist(user.address)
      ).not.to.emit(blacklistable, EVENT_NAME_UNBLACKLISTED);
    });

    it("Is reverted if called not by the blacklister", async () => {
      const { blacklistable } = await setUpFixture(deployAndConfigureBlacklistable);
      await expect(
        blacklistable.connect(user).unBlacklist(user.address)
      ).to.be.revertedWithCustomError(blacklistable, REVERT_ERROR_UNAUTHORIZED_BLACKLISTER);
    });
  });
  ...
  describe("Function 'selfBlacklist()'", async () => {
    it("Executes as expected and emits the correct events if it is called by any account", async () => {
      ...
      expect(await blacklistable.isBlacklisted(user.address)).to.equal(false);
      await expect(
        blacklistable.connect(user).selfBlacklist()
      ).to.emit(
        blacklistable,
        EVENT_NAME_BLACKLISTED
      ).withArgs(
      ...
      await expect(
        blacklistable.connect(user).selfBlacklist()
      ).not.to.emit(blacklistable, EVENT_NAME_SELFBLACKLISTED);
    });
  });
  ...
  describe("Modifier 'notBlacklisted'", async () => {
    it("Is not reverted if the caller is not blacklisted", async () => {
      const { blacklistable } = await setUpFixture(deployAndConfigureBlacklistable);
      await expect(
        blacklistable.connect(user).testNotBlacklistedModifier()
      ).to.emit(blacklistable, EVENT_NAME_TEST_NOT_BLACKLISTED_MODIFIER_SUCCEEDED);
    });
    ...
    it("Is reverted if the caller is blacklisted", async () => {
      ...
      await expect(
        blacklistable.connect(user).testNotBlacklistedModifier()
      ).to.be.revertedWithCustomError(blacklistable, REVERT_ERROR_BLACKLISTED_ACCOUNT);
    });
  });
});
