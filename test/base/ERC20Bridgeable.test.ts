...
describe("Contract 'ERC20Bridgeable'", async () => {
    ...
    const REVERT_MESSAGE_ERC20_MINT_TO_THE_ZERO_ADDRESS =
        "ERC20: mint to the zero address";
    const REVERT_MESSAGE_ERC20_BURN_FROM_THE_ZERO_ADDRESS =
    ...
    describe("Function 'mintForBridging()'", async () => {
        it("Executes as expected and emits the correct event", async () => {
            const { token } = await setUpFixture(deployToken);
            await expect(
                token.connect(bridge1).mintForBridging(user.address, TOKEN_AMOUNT)
            )
        ...
        it("Is reverted if called to mint for the zero address", async () => {
            ...
            await expect(
                token
                    .connect(bridge1)
                    .mintForBridging(ethers.constants.AddressZero, TOKEN_AMOUNT)
    ...
    describe("Function 'burnForBridging()'", async () => {
        it("Executes as expected and emits the correct event", async () => {
            ...
            await expect(
                token.connect(bridge1).burnForBridging(user.address, TOKEN_AMOUNT)
            )
                .to.changeTokenBalances(
                    token,
                    [deployer, bridge1, token, user],
        ...
        it("Is reverted if called to burn from the zero address", async () => {
            ...
            await expect(
                token
                    .connect(bridge1)
                    .burnForBridging(ethers.constants.AddressZero, TOKEN_AMOUNT)
