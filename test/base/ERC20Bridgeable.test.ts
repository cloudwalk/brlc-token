...
describe("Contract 'ERC20Bridgeable'", async () => {
  ...
  describe("Function 'mintForBridging()'", async () => {
    it("Executes as expected and emits the correct event", async () => {
      const { token } = await setUpFixture(deployToken);
      const tx = (token.connect(bridge1) as Contract).mintForBridging(user.address, TOKEN_AMOUNT);
    ...
    it("Is reverted if called not by the bridge", async () => {
      const { token } = await setUpFixture(deployToken);
      await expect(
        (token.connect(user) as Contract).mintForBridging(user.address, TOKEN_AMOUNT)
    ...
    it("Is reverted if called to mint for the zero address", async () => {
      const { token } = await setUpFixture(deployToken);
      await expect(
        (token.connect(bridge1) as Contract).mintForBridging(ethers.ZeroAddress, TOKEN_AMOUNT)
    ...
    it("Is reverted if the token minting amount is zero", async () => {
      const { token } = await setUpFixture(deployToken);
      await expect(
        (token.connect(bridge1) as Contract).mintForBridging(user.address, 0)
  ...
  describe("Function 'burnForBridging()'", async () => {
    it("Executes as expected and emits the correct event", async () => {
      const { token } = await setUpFixture(deployToken);
      await proveTx((token.connect(bridge1) as Contract).mintForBridging(user.address, TOKEN_AMOUNT));
      const tx = (token.connect(bridge1) as Contract).burnForBridging(user.address, TOKEN_AMOUNT);
      await expect(tx).to.changeTokenBalances(
        token,
        [deployer, bridge1, token, user],
        [0, 0, 0, -TOKEN_AMOUNT]
    ...
    it("Is reverted if called not by the bridge", async () => {
      ...
      await proveTx((token.connect(bridge1) as Contract).mintForBridging(user.address, TOKEN_AMOUNT));
      await expect(
        (token.connect(user) as Contract).burnForBridging(user.address, TOKEN_AMOUNT)
      ).to.be.revertedWithCustomError(token, REVERT_ERROR_UNAUTHORIZED_BRIDGE);
    });
    ...
    it("Is reverted if called to burn from the zero address", async () => {
      const { token } = await setUpFixture(deployToken);
      await expect(
        (token.connect(bridge1) as Contract).burnForBridging(ethers.ZeroAddress, TOKEN_AMOUNT)
    ...
    it("Is reverted if called to burn more tokens than the bridge balance", async () => {
      const { token } = await setUpFixture(deployToken);
      await expect(
        (token.connect(bridge1) as Contract).burnForBridging(user.address, TOKEN_AMOUNT + 1)
    ...
    it("Is reverted if the token burning amount is zero", async () => {
      const { token } = await setUpFixture(deployToken);
      await expect(
        (token.connect(bridge1) as Contract).burnForBridging(user.address, 0)
  ...
  describe("Function 'setBridge()'", async () => {
    ...
    it("Is reverted if called not by the owner", async () => {
      ...
      expect(await token.bridge()).to.eq(bridge1.address);
      await expect(
        (token.connect(user) as Contract).setBridge(bridge2.address)
