...
describe("Contract ERC20Trustable", async () => {
    ...
    const MAXIMUM_APPROVE_VALUE = ethers.constants.MaxUint256;
    const APPROVE_VALUE = 123;

    const EVENT_TRUSTED_CONFIGURED = "ConfigureTrusted";

    const REVERT_ERROR_TRUSTED_ALREADY_CONFIGURED = "TrustedAlreadyConfigured";
    ...
    describe("Function configureTrusted()", async () => {
        it("Executes as expected and emits the event", async () => {
            const { token } = await setUpFixture(deployToken);

            expect(await token.isTrusted(trustedAccount.address)).to.eq(false);

            expect(await token.configureTrusted(trustedAccount.address, true))
                .to.emit(token, EVENT_TRUSTED_CONFIGURED)
                .withArgs(trustedAccount, true);
            expect(await token.isTrusted(trustedAccount.address)).to.eq(true);

            expect(await token.configureTrusted(trustedAccount.address, false))
                .to.emit(token, EVENT_TRUSTED_CONFIGURED)
        ...
        it("Is reverted if the account is already configured", async () => {
            ...
            await proveTx(token.configureTrusted(trustedAccount.address, true));
            expect(token.configureTrusted(trustedAccount.address, true))
                .to.be.revertedWithCustomError(token, REVERT_ERROR_TRUSTED_ALREADY_CONFIGURED);

            await proveTx(token.configureTrusted(trustedAccount.address, false));
            expect(token.configureTrusted(trustedAccount.address, false))
                .to.be.revertedWithCustomError(token, REVERT_ERROR_TRUSTED_ALREADY_CONFIGURED);
        });

        it("Is reverted if the caller is not an owner", async () => {
            const { token } = await setUpFixture(deployToken);
            expect(token.connect(user).configureTrusted(trustedAccount.address, true))
    ...
    describe("Function allowance()", async () => {
        it("Returns correct allowance if spender is marked as trusted", async () => {
            ...
            await proveTx(token.connect(user).approve(trustedAccount.address, APPROVE_VALUE));
            expect(await(token.allowance(user.address, trustedAccount.address)))
                .to.eq(APPROVE_VALUE);

            await proveTx(token.configureTrusted(trustedAccount.address, true));
            expect(await(token.allowance(user.address, trustedAccount.address)))
                .to.eq(MAXIMUM_APPROVE_VALUE);

            await proveTx(token.connect(user).approve(trustedAccount.address, APPROVE_VALUE * 2));
            expect(await(token.allowance(user.address, trustedAccount.address)))
                .to.eq(MAXIMUM_APPROVE_VALUE);

            await proveTx(token.configureTrusted(trustedAccount.address, false));
            expect(await(token.allowance(user.address, trustedAccount.address)))
                .to.eq(APPROVE_VALUE * 2);
        });
    });
});