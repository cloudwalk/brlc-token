...
abstract contract ERC20Restrictable is ERC20Base, IERC20Restrictable {
     ...
     * @inheritdoc IERC20Restrictable
     */
    function restrictionIncrease(address account, bytes32 purpose, uint256 amount) external onlyOwner {
     ...
     * @inheritdoc IERC20Restrictable
     */
    function restrictionDecrease(address account, bytes32 purpose, uint256 amount) external onlyOwner {
        if (account == address(0)) {
            revert ZeroAddress();
        }
        if (purpose == bytes32(0)) {
